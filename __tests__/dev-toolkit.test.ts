...
describe('scripts/dev-toolkit.mjs', () => {
  ...
  it('lists every harness subcommand (currently 16)', async () => {
    // iter 69 → 15. iter 98 → 16 with export-config (iter 97).
    // Every dispatcher-honoured subcommand MUST show up here or new
    // contributors won't discover the tool that solves their problem.
    const r = await run();
    const subs = [
